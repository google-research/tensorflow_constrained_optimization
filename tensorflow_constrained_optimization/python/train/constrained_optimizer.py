...
class ConstrainedOptimizer(object):
  ...
  applies it to a `ConstrainedMinimizationProblem`. Unlike a
  `tf.train.Optimizer`, which takes a `Tensor` to minimize as a parameter to its
  minimize() method, a constrained optimizer instead takes a
  ...
  def minimize(self,
    ...
    perform an unconstrained update, and if "global_step" >=
    "unconstrained_steps", it will perform a constrained update.
